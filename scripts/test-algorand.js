...
const commands = {
  ...
  async wallet() {
    ...
    if (!walletAddress) {
      ...
      console.log('💡 Make sure .env.local contains: VITE_APP_WALLET_ADDRESS=your_address');
      process.exit(1);
    }
    ...
    try {
      ...
      if (balance === 0) {
        ...
        console.log('   For testnet: Visit https://testnet.algoexplorer.io/dispenser');
  ...
  async credential() {
    ...
    try {
      ...
      const testCredential = {
        ...
        "credentialSubject": {
          "id": "did:cardless:user:test",
          "cardless:compositeHash": "test-hash"
        }
      };
      ...
      const qrDataURL = await QRCode.default.toDataURL(JSON.stringify(testCredential));

...
- The base framework for testing is [vitest](https://vitest.dev/), and the project includes two separate kinds of tests:
  - `Algorand TypeScript` smart contract unit tests (`.algo.spec.ts` files), that are run using [`algorand-typescript-testing`](https://github.com/algorandfoundation/algorand-typescript-testing), which are executed in a Node.js interpreter emulating major AVM behaviour. These tests are fast and don't require LocalNet.
  - End-to-end (e2e) `AppClient` tests (`.e2e.spec.ts` files) that are run against `algokit localnet` and test the behaviour in a real network environment
...
The project includes separate test commands for different testing scenarios:

- **`npm test`** - Run all tests (unit and e2e) with coverage. *Note: Requires LocalNet to be running.*
- **`npm run test:unit`** - Run only unit tests (fast, no LocalNet required)
- **`npm run test:e2e`** - Run e2e tests (automatically starts LocalNet)
- **`npm run test:watch`** - Run tests in watch mode for development

To run tests manually:
```bash
# Run only unit tests (no LocalNet needed)
npm run test:unit

# Run e2e tests (auto-starts LocalNet)
npm run test:e2e
